...
`phase4-prod-ingress-implementation-plan-rc1` defines the future production edge path for load balancer or API Gateway selection, serverless NEG routing, TLS certificates, mTLS trust, Cloud Armor policy, admin OIDC, private Cloud Run ingress, request-path separation, required apply gates, and a design-only Terraform guard. It creates no ingress resources, changes no Cloud Run ingress settings, exposes no public endpoint, and enables no production ingress. Details are recorded in `PHASE4_PROD_INGRESS_IMPLEMENTATION_PLAN.md`.

`phase4-dr-readiness-rc1` defines the disaster recovery readiness path for Cloud SQL backup and PITR evidence, restore-to-separate-target drills, ledger reconciliation checks, validator recovery, secret recovery, RTO/RPO targets, required drill gates, and a design-only Terraform guard. It executes no restore drill, creates no restore instance, changes no backup settings, and enables no production recovery. Details are recorded in `PHASE4_DR_READINESS.md`.

`phase4-legal-compliance-evidence-rc1` defines the evidence register for legal classification, registration/MSB analysis, AML/KYC, sanctions, privacy, custody, institution agreements, security, penetration testing, incident response, production cost, Terraform plan, and executive go-live review. It provides no legal advice, approves no gate, onboards no external institution, and enables no production or real-value capability. Details are recorded in `PHASE4_LEGAL_COMPLIANCE_EVIDENCE.md`.

`phase4-wrapup-rc1` closes the Phase 4 no-GKE pre-production readiness track. It confirms every Phase 4 checkpoint is present, all production and real-value gates remain blocked, GKE remains deferred to `phase5b-gke-validator-ops`, and the next implementation track is `phase5a-no-gke-validator-hardening`. Details are recorded in `PHASE4_WRAPUP.md`.
