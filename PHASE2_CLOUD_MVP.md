...
- Basic ISO 20022 `pacs.008` ingestion maps one sandbox credit transfer into the ledger payment flow.
...
- Basic `pacs.008` parser in `crates/kani-iso20022` and API endpoint `POST /v1/iso20022/pacs008`.
...
The smoke test exercises both the JSON payment endpoint and the ISO 20022 `pacs.008` endpoint against a fresh sandbox test asset.
...
- ISO 20022 support is limited to one sandbox `pacs.008` credit transfer per XML document; `pacs.002` and `camt.*` are still deferred.
