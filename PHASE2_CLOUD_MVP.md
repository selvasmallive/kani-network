...
- Basic ISO 20022 `pacs.008` ingestion maps one sandbox credit transfer into the ledger payment flow, and `pacs.002` reports the resulting payment status.
...
- Basic `pacs.008` parser plus `pacs.002` status generator in `crates/kani-iso20022`; API endpoints are `POST /v1/iso20022/pacs008` and `GET /v1/iso20022/pacs002/{payment_id}`.
...
The smoke test exercises the JSON payment endpoint, the ISO 20022 `pacs.008` endpoint, and the ISO 20022 `pacs.002` status endpoint against a fresh sandbox test asset.
...
- ISO 20022 support is limited to one sandbox `pacs.008` credit transfer per XML document and basic `pacs.002` payment status XML; `camt.*` is still deferred.
